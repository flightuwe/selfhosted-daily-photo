...
class ReleaseHistoryRepositoryTest {
    ...
    fun manualRefreshBypassesFreshCacheAndPersistsTheNewHistory() = runBlocking {
        var fetched = listOf(entry("0.8.16"))
        val repository = ReleaseHistoryRepository(context, releaseFetcher = { fetched })
        repository.history(allowNetwork = true)
        fetched = listOf(entry("0.8.22"), entry("0.8.16"))
    ...
    fun failedOnlineRefreshKeepsTheExistingCacheAndMarksItStale() = runBlocking {
        ...
        val result = repository.history(allowNetwork = true, forceRefresh = true)
