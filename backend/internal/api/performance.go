...
func (s *Server) handleAdminPerformanceOverview(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"slo": slo,
		"summary": gin.H{
			"requests": sumInt64FromBuckets(buckets, func(item performanceBucketRow) int64 { return item.Requests }),
			"errors":   sumInt64FromBuckets(buckets, func(item performanceBucketRow) int64 { return item.Errors }),
...
func (s *Server) handleAdminPerformanceRoutes(c *gin.Context) {
	...
	aggMap := make(map[string]*routeAgg, 256)
	for _, row := range rows {
		key := row.Method + "|" + row.Route
		item, exists := aggMap[key]
		if !exists {
			item = &routeAgg{Route: row.Route, Method: row.Method}
			aggMap[key] = item
		}
	...
	out := make([]routeAgg, 0, len(aggMap))
	...
	for _, row := range out {
		errorRate := 0.0
		if row.Requests > 0 {
			errorRate = float64(row.Errors) / float64(row.Requests)
		}
		items = append(items, gin.H{
			...
			"errors":       row.Errors,
			"errors4xx":    row.Errors4x,
			"errors5xx":    row.Errors5x,
			"errorRate":    errorRate,
			"p95PeakMs":    row.P95Peak,
			"p99PeakMs":    row.P99Peak,
			"maxPeakMs":    row.MaxPeak,
		})
	}
	c.JSON(http.StatusOK, gin.H{
		"from":  from,
